...
export class RegisterComponent implements OnInit{
  ...
  submitForm() {
    this.http.get(` http://localhost:8081/user/is-exists-user/${this.userObject.username}`).subscribe((data) => {
  ...
  registerUser(condition:boolean){
    if(condition){
      ...
      this.http.post("http://localhost:8081/user/add-user", this.userObject).subscribe(
        (data) => {
